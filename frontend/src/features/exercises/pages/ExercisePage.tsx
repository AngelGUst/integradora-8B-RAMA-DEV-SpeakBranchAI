...
import {
  ArrowLeft, BookOpen, Mic, Repeat, Headphones,
  CheckCircle2, XCircle, Volume2, CircleStop, Loader2,
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { EXERCISES } from '../data/exerciseData';
import type {
  AnyExercise, ReadingExercise, SpeakingExercise, ComprehensionExercise, MCQQuestion,
...
export default function ExercisePage() {
  ...
  const exercise: AnyExercise | undefined = EXERCISES[id ?? ''];
  const [started, setStarted] = useState(false);

  if (!exercise) return <Navigate to="/learn" replace />;

  const meta = SKILL_META[exercise.skill as keyof typeof SKILL_META];
  const { Icon } = meta;

  const handleComplete = (_score: number, xp: number) => {
    completeExercise(exercise.id, xp);
    // navigate is called by the player's onBack via ResultScreen button
  };
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-semibold ${meta.border} ${meta.bg} ${meta.color}`}>
            <Icon size={12} />
            {meta.label} · {exercise.level}
          </div>
          <span className="ml-auto text-xs text-zinc-600 font-mono">max {exercise.maxXP} XP</span>
        </div>

        <h1 className="text-xl font-semibold text-zinc-100 mb-1">{exercise.title}</h1>
        <p className="text-sm text-zinc-500 mb-6">
          {exercise.type === 'reading'       && 'Lee el texto y responde las preguntas de opción múltiple.'}
          {exercise.type === 'speaking'      && (exercise as SpeakingExercise).instruction}
          {exercise.type === 'comprehension' && 'Escucha el audio y responde las preguntas sin ver el texto.'}
        </p>
        ...
        {!started && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
              ...
              <p className="font-semibold text-zinc-100">{exercise.title}</p>
              <p className="text-sm text-zinc-500 mt-0.5 mb-4">{meta.label} · {exercise.level}</p>
                ...
                {exercise.type !== 'speaking' && (
                  <div>
                    ...
                    <p className="text-lg font-bold text-zinc-200">
                      {(exercise as ReadingExercise | ComprehensionExercise).questions.length}
                    </p>
                  </div>
                )}
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-zinc-600">XP máximo</p>
                  <p className={`text-lg font-bold ${meta.color}`}>{exercise.maxXP}</p>
                </div>
                {exercise.type === 'comprehension' && (
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-zinc-600">Reproducciones</p>
                    <p className="text-lg font-bold text-zinc-200">{(exercise as ComprehensionExercise).maxReplays}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {started && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {exercise.type === 'reading' && (
              <ReadingPlayer
                ex={exercise as ReadingExercise}
                onComplete={(s, xp) => { handleComplete(s, xp); goLearn(); }}
              />
            )}
            {exercise.type === 'speaking' && (
              <SpeakingPlayer
                ex={exercise as SpeakingExercise}
                onComplete={(s, xp) => { handleComplete(s, xp); goLearn(); }}
              />
            )}
            {exercise.type === 'comprehension' && (
              <ComprehensionPlayer
                ex={exercise as ComprehensionExercise}
                onComplete={(s, xp) => { handleComplete(s, xp); goLearn(); }}
              />
            )}
          </motion.div>
